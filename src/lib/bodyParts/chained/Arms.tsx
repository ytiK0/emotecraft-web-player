...
const LEFT_ARM_TEXTURE_CONFIG: TextureConfig = {
  textureStart: new Vector2(32, 64 - 16),
};
const LEFT_ARM_BEND_TEXTURE_START: TextureConfig = {
  textureStart: new Vector2(32, 64 - 16),
  sidesStart: new Vector2(32, 64 - 6),
  textureSizes: new Vector3(...ARM_SIZE)
};
export const LeftArm = forwardRef<DisposableBodyPartRepresentation | undefined, BodyPartProps>(({children, name, debug, position}: BodyPartProps, ref) => {
  ...
  return (
    <BodyPartBase
      ...
      textureConfig={LEFT_ARM_TEXTURE_CONFIG}
      name={name}
      debug={debug}
      position={position}
    >
      <BodyPartBase
        ...
        partSize={ARM_SIZE}
        textureConfig={LEFT_ARM_BEND_TEXTURE_START}
        name={`${name}_bend`}
        debug={debug}
...
const RIGHT_ARM_TEXTURE_CONFIG: TextureConfig = {
  textureStart: new Vector2(32 + 8, 16)
};
const RIGHT_ARM_BEND_TEXTURE_CONFIG: TextureConfig = {
  textureStart: new Vector2(32 + 8, 16),
  sidesStart: new Vector2(32 + 8, 16 + 10),
  textureSizes: new Vector3(...ARM_SIZE)
};
export const RightArm = forwardRef<DisposableBodyPartRepresentation | undefined, BodyPartProps>(({children, name, debug, position}: BodyPartProps, ref) => {
  ...
  return (
    <BodyPartBase
      ...
      partSize={ARM_SIZE}
      textureConfig={RIGHT_ARM_TEXTURE_CONFIG}
      name={name}
      debug={debug}
      position={position}
    >
      <BodyPartBase
        ...
        pivotShift={ARM_BEND_PIVOT_POINT_SHIFT}
        partSize={ARM_BEND_SIZE}
        textureConfig={RIGHT_ARM_BEND_TEXTURE_CONFIG}
        name={`${name}_bend`}
        debug={debug}
